...
// MockStorageEngine is a mock implementation of StorageEngineInterface for testing.
// It uses testify/mock to allow setting expectations and asserting calls.
// It should be used across different packages for testing components that interact with the engine.
...
var _ StorageEngineInterface = (*MockStorageEngine)(nil)

func (m *MockStorageEngine) GetNextSSTableID() uint64 {
	// Allow tests to mock this or use a default behavior.
	args := m.Called()
	if fn, ok := args.Get(0).(func() uint64); ok {
		return fn()
	}
...
func (m *MockStorageEngine) GetSequenceNumber() uint64 {
	args := m.Called()
	// Provide a default return value if not mocked, to avoid panics.
	if len(args) == 0 {
		return 0
	}
...
func (m *MockStorageEngine) GetPubSub() (PubSubInterface, error) {
	args := m.Called()
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(PubSubInterface), args.Error(1)
}
...
func (m *MockStorageEngine) Metrics() (*EngineMetrics, error) {
	args := m.Called()
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*EngineMetrics), args.Error(1)
}

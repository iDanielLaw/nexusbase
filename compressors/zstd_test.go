...
func TestZSTDCompressor(t *testing.T) {
	...
	if compressor.Type() != core.CompressionZSTD {
		t.Errorf("ZSTDCompressor.Type() got = %v, want %v", compressor.Type(), core.CompressionZSTD)
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(tc.data) > 0 && len(compressed) >= len(tc.data) && tc.name == "repetitive data" {
				t.Logf("Warning: ZSTD compression did not reduce data size for repetitive data. Original: %d, Compressed: %d", len(tc.data), len(compressed))
			}
		})
	}
}
...
func BenchmarkZSTDCompressTo(b *testing.B) {
	compressor := NewZstdCompressor()
	data := []byte(`{"metric":"cpu.usage","tags":{"host":"server-a","region":"us-east-1"},"timestamp":1678886400000000000,"fields":{"value":99.8}}`)
	data = bytes.Repeat(data, 50)

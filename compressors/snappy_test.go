...
func TestSnappyCompressor(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "simple string",
			data: []byte("hello world, this is a test of the snappy compressor"),
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(tc.data) > 0 && len(compressed) >= len(tc.data) && tc.name == "repetitive data" {
				t.Logf("Warning: Snappy compression did not reduce data size for repetitive data. Original: %d, Compressed: %d", len(tc.data), len(compressed))
			}
		})
	}
}

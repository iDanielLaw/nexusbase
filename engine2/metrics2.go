...
// EngineMetrics2 is the engine2-local metrics struct. It's intentionally
// distinct from the legacy `engine.EngineMetrics` so we can migrate callers
// incrementally. Conversion helpers live in `metrics_convert.go`.
type EngineMetrics2 struct {
...
// NewEngineMetrics2 creates a new engine2-local EngineMetrics2.
func NewEngineMetrics2(publishGlobally bool, prefix string) *EngineMetrics2 {
	...
	// Initialize histogram maps with count/sum and buckets. Reuse
	// package-level latencyBuckets (defined in other engine2 helpers).
	histMaps := []*expvar.Map{
		em.FlushLatencyHist,
...
func (em *EngineMetrics2) GetActiveSeriesCount() (int, error) {

...
func (s *GRPCServer) CreateSnapshot(ctx context.Context, req *tsdb.CreateSnapshotRequest) (*tsdb.CreateSnapshotResponse, error) {
	...
	// The snapshot directory is relative to the engine's configured snapshots base directory.
	// The engine handles the full path construction.
	err := s.engine.CreateSnapshot(req.GetSnapshotDir())
	if err != nil {
